...
def action(choice):
    match choice:
        ...
        case 3:
            ...
            ST= int(input("Enter the strategy number:"))
...
# # from TestBOt.pythondemo import client
# from binance_api import create_client
# from strategy import SUPER_TREND
# from datetime import datetime

# client = create_client()
# global S  # Global symbol variable if needed
...
#             print(f"Available USDT balance: {balance['free']}")
#             print(" ")
...
#                         total_buy += cost
#                         buy_qty += qty
#                         print(f"[BUY ] {SYMBOL} | Time: {time_str} | Qty: {qty:.6f} | Price: ${price:.2f} | Cost: ${cost:.2f}")
#                     else:
#                         total_sell += cost
#                         sell_qty += qty
#                         print(f"[SELL] {SYMBOL} | Time: {time_str} | Qty: {qty:.6f} | Price: ${price:.2f} | Revenue: ${cost:.2f}")
...
#                 # balance_info = client.get_asset_balance(asset=base_asset)
#                 # quantity_held = float(balance_info['free'])
#                 # print(f"Net Quantity:  {quantity_held:.6f}")
#                 # print(f"Estimated PnL: ${net_pnl:.2f}")
...
#             print("Top 50 tradable pairs on Testnet:")
#             for i, symbol in enumerate(top_500[:50], start=1):
#                 print(f"{i}. {symbol}")
